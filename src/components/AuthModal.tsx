...
import { registrarUsuario, loginUsuario } from '../apiAuth';
...
export function AuthModal({ isOpen, onClose, onLogin }: AuthModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await loginUsuario({ email: loginEmail, password: loginPassword });
      toast.success('¡Bienvenido de nuevo!');
      onLogin({ id: data.usuario.id, name: data.usuario.nombre, email: data.usuario.email, phone: '' });
      onClose();
      resetForms();
    } catch (err: any) {
      toast.error(err.message || 'Credenciales incorrectas. Verifica tu email y contraseña.');
    }
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await registrarUsuario({ nombre: registerName, email: registerEmail, password: registerPassword });
      ...
      toast.error(error.message || 'Hubo un error al crear la cuenta. Inténtalo de nuevo.');
    } finally {
